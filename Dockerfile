...
# in under 30 seconds.
#
# Build: docker build -t tml-framework .
# Run:   docker run -it tml-framework
...
# Memorial message for framework creator
...
CMD ["/bin/bash", "-c", "cat /home/tml/.tml_info && echo && echo 'TML Framework Ready! Try these commands:' && echo '  python tests/isolated_test.py              # Run basic TML tests' && echo '  python examples/medical_ai_triage.py       # Medical AI demo' && echo '  python examples/autonomous_vehicle.py      # Autonomous vehicle demo' && echo '  python examples/content_moderation.py      # Content moderation demo' && echo '  python examples/financial_ai.py            # Financial AI demo' && echo '  pytest tests/ -v                          # Run comprehensive tests' && echo '  python -c \"from tml_core import *; print(\\\"TML imported successfully\\\")\"' && echo && echo 'For interactive exploration:' && echo '  python                                     # Start Python REPL' && echo '  jupyter notebook --ip=0.0.0.0 --no-browser # Start Jupyter (if needed)' && echo && /bin/bash"]
