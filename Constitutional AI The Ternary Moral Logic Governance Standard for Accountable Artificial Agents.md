# **Constitutional AI: The Ternary Moral Logic Governance Standard for Accountable Artificial Agents** {#constitutional-ai:-the-ternary-moral-logic-governance-standard-for-accountable-artificial-agents}
...
**[Constitutional AI: The Ternary Moral Logic Governance Standard for Accountable Artificial Agents	1](#constitutional-ai:-the-ternary-moral-logic-governance-standard-for-accountable-artificial-agents)**
...
[Author: Lev Goukassian	1](#author:-lev-goukassian)
...
[11.4.2 Nonce-Leak-Resistant Ephemeral Key Rotation (Side-Channels and Latency)	127](#11.4.2-ephemeral-key-rotation:-side-channels-and-latency)
...
#### **11.4.2 Nonce-Leak-Resistant Ephemeral Key Rotation (Side-Channels and Latency)** {#11.4.2-ephemeral-key-rotation:-side-channels-and-latency}
...
**Construction – “Hash-DRBG + Synthetic Nonce”**
    ...
    - TEE_RNG: 256-bit fresh entropy from the on-die RNG (RDRAND / DIT-RNG)  

    - epoch_id: monotonic counter stored in TPM NV-index (anti-rollback)  

    - heartbeat_sequence: the same 64-bit counter already in the log header  

    - log_hash: SHA-256 of the exact payload being signed – binds the nonce to the message
    ...
    Security Argument: Even if the TEE RNG is biased, the blake3 construction acts as a strong randomness extractor; uniqueness of (epoch_id, heartbeat_sequence) prevents duplicate nonces under high load; inclusion of log_hash eliminates the classic Sony PS3 “same message” reuse vector.
...
**Operational Procedures**
    ...
    - Heartbeat-sequence gaps >1 trigger automatic “key rotation” (new epoch_id) to bound exposure if an RNG failure goes unnoticed.  

    - Emergency reseed: if RDRAND returns all-zero (hardware failure), signer returns `Err("RNG_DEAD")` → system must halt (Fail-Closed), preserving the “No Log = No Action” invariant.

**Security Proof Sketch**

    Under the blake3-PRF assumption and uniqueness of (epoch_id, heartbeat), the probability that two distinct messages produce the same nonce is ≈ 2⁻²⁵⁶. Even given 2⁶⁴ signatures, the expected number of collisions is < 2⁻¹²⁸, far below the 2⁻¹²⁸ Ed25519 security target. Timing leakage is eliminated because the code path is identical for every signing call (constant-time blake3 + constant-time dalek).

**Migration Path**

    - Drop-in replacement for `ed25519_dalek::SigningKey::sign()` – no wire-format change.  

    - Old logs remain valid; new logs carry an extra header flag `SIG_VER=1` so auditors know which verification equation to apply.  
